...
|---------|---------|-------|--------|--------|
| **Gantt chart** | — | ✅ Full | ⚠️ Roadmap now has dependency lines, resize handles, milestone-style markers for due-date-only items, draggable date-range bars, real month/week timeline buckets, timeline window navigation, zoom controls, a sticky issue sidebar, a unified today marker, epic summary bars, epic/status/assignee/priority grouping, collapsible swimlanes, and a collapsible parent/subtask tree; still lacks full dedicated Gantt experience | High — standard PM expectation |
