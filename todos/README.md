...
- Priority `01` update (2026-03-02): CI summary reliability now includes deterministic self-tests (`pnpm run e2e:summary:self-test`), exact scan-window accounting (`scanned/limit`), truncation signaling for capped-clean streaks, and negative assertions that block false-positive truncation notes in fallback/non-truncated history paths. CI hard rules now also block `waitForLoadState("networkidle")` in spec files (alongside timeout and brittle selector regression checks). Current validated contracts remain `fallback-local: 1/5` and mock `history-derived: 2/5` with `3/250` scan-window reporting plus truncation-case validation (`2/2`). Selector baseline debt remains zero. Remaining blocker is one live PR CI validation against real Actions API + step summary output.
