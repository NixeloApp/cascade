...
- Priority `01` update (2026-03-02): CI summary reliability now includes deterministic self-tests (`pnpm run e2e:summary:self-test`) with validated fallback contracts for both clean (`1/5`) and dirty (`0/5`) runs, exact scan-window accounting (`scanned/limit`), truncation signaling for capped-clean streaks, negative assertions that block false-positive truncation notes in fallback/non-truncated history paths, strict scan-limit parsing for `E2E_STREAK_SCAN_LIMIT` (accepts only canonical positive integers; invalid/non-positive/mixed/ambiguous tokens like `not-a-number`, `0`, `-7`, `250abc`, `010`, and oversized integers fall back to `100`; trimmed valid values remain supported), and a hard safety cap of `1000` runs for valid large scan-limit inputs (`5000` -> `1000`). CI hard rules block `waitForTimeout(...)`, `waitForLoadState("networkidle")`, and brittle selector regressions with deterministic checker self-tests (`pnpm run e2e:hard-rules:self-test`) wired into CI. Validated mock history contracts now cover clean, dirty, failing-first latest-run, and no-e2e-jobs scenarios (`history-derived` `2/5` and `0/5`), including `3/250`, `1/250`, and `2/250` scan-window behavior plus truncation-case validation (`2/2`). `e2e-summary` now emits explicit errors for missing, invalid, and unreadable JSON in both report and mock-history inputs and has verified step-summary file write/append behavior. Selector baseline debt remains zero. Remaining blocker is one live PR CI validation against real Actions API + step summary output.
