...
- [ ] [visual-consistency-hardening.md](./visual-consistency-hardening.md) — Screenshot-driven product polish and human review debt; the shared page header/content shell cleanup plus the flatter `OverviewBand` cleanup are now in, the shared project/workspace/team detail shells now use lighter headers and compact section strips, the public auth suite now uses a shared email-reveal shell with current reviewed specs, meetings now uses focused small-screen detail panes instead of a stacked transcript/action-item wall, dashboard/projects/invoices/outreach/documents/issues/my-issues/notifications/roadmap/time-tracking/project-inbox/workspaces/org-calendar/project-analytics/org-analytics/assistant coverage is now in-spec, but broader page-by-page cleanup remains.
...
- [ ] [offline-pwa.md](./offline-pwa.md) — Push verification plus worker/runtime proof remain; queued issue status, comments, and notification reads now apply optimistic UI immediately from the offline queue.
...
| App tests (`vitest run`) | 4671 pass / 3 skipped |
