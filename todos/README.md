...
- Priority `01` update (2026-03-02): CI summary reliability now includes deterministic self-tests (`pnpm run e2e:summary:self-test`) with validated fallback contracts for both clean (`1/5`) and dirty (`0/5`) runs, exact scan-window accounting (`scanned/limit`), truncation signaling for capped-clean streaks, and negative assertions that block false-positive truncation notes in fallback/non-truncated history paths. CI hard rules block `waitForTimeout(...)`, `waitForLoadState("networkidle")`, and brittle selector regressions with deterministic checker self-tests (`pnpm run e2e:hard-rules:self-test`) wired into CI. Validated mock history contracts now cover clean, dirty, and failing-first latest-run scenarios (`history-derived` `2/5` and `0/5`), including `3/250` and `1/250` scan-window behavior plus truncation-case validation (`2/2`). Selector baseline debt remains zero. Remaining blocker is one live PR CI validation against real Actions API + step summary output.
