...
- [ ] The remaining debt is no longer one giant screenshot-lib selector hotspot. What is left is smaller but still structural: screenshot-specific helper seams, the blocked transport helpers themselves, and the remaining places where screenshot capture can still diverge from reusable E2E flows.
...
- [ ] Keep shrinking direct screenshot-lib route-driving so ordinary canonical, modal, and loading captures go through page-object navigation, leaving only the blocked transport helpers themselves as the justified exception.
