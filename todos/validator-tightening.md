...
- Tightened `check-colors.js` again by moving the shared preset/editor/Y.js palettes into `convex/shared/colors.ts`, removing `ColorPicker.tsx`, `Plate/ColorPickerButton.tsx`, `src/lib/yjs/awareness.ts`, and `src/lib/yjs/ConvexYjsProvider.ts` from the hardcoded-color allowlist so the validator now treats that shared palette module as the single intentional runtime color boundary.
- Tightened `check-colors.js` again by moving the remaining seeded/runtime status colors for `pumble`, `invites`, `bookingPages`, `projectTemplates`, and `onboarding` into `convex/shared/colors.ts`, removing those Convex product files from the hardcoded-color allowlist so runtime color policy is now concentrated in one shared module instead of scattered across multiple backend surfaces.
- Tightened `check-colors.js` again by moving OAuth popup HTML styling behind a shared `convex/http/oauthHtml.ts` helper backed by `convex/shared/colors.ts`, removing `convex/http/githubOAuth.ts` and `convex/http/googleOAuth.ts` from the hardcoded-color allowlist so the validator no longer treats OAuth handlers as a special final color island.
- Hardened `check-colors.js` after product/runtime color debt hit zero: replaced the remaining allowlist framing with explicit owned color boundaries (`src/index.css`, `convex/shared/colors.ts`, and test-only cases), so color validation now reads as final enforcement policy instead of migration-mode exception handling.
