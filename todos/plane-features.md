...
- [~] **Evaluate Gantt implementation** — Our roadmap now supports draggable bars, resize handles, dependency lines, real month/week timeline buckets, timeline window navigation, zoom controls, a sticky issue sidebar, a unified today marker, epic/status/assignee/priority grouping, and collapsible swimlanes, but still needs fuller Gantt affordances (sidebar polish/deeper timeline controls).
