...
- [~] **Evaluate Gantt implementation** — Our roadmap now supports draggable bars, resize handles, milestone-style markers for due-date-only items, dependency lines, real month/week timeline buckets, timeline window navigation, zoom controls, a sticky issue sidebar, a unified today marker, epic summary bars, epic/status/assignee/priority grouping, collapsible swimlanes, and nested parent/subtask rows, but still needs fuller Gantt affordances (sidebar polish/deeper timeline controls).
