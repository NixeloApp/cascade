...
- [~] **Evaluate Gantt implementation** — Our roadmap now supports draggable bars, resize handles, dependency lines, real month/week timeline buckets, timeline window navigation, zoom controls, group-by sections, and collapsible swimlanes, but still needs fuller Gantt affordances (sidebar polish/deeper timeline controls).
