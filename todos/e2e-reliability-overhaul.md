...
1. Deterministic post-action assertions in critical flows:
   ...
   - `InvitePage.expectLoadingOrInvalid()` now polls a named invite-route state (`loading` / `invalid` / `pending`) instead of a coarse boolean "did anything show up" check, so the public invite loading assertion waits on an explicit route outcome; the focused `invite.spec.ts` rerun stayed green (`4 passed`, `29.3s`).
...
- `ensureAuthenticated()` now routes authenticated tests through `/app` and waits for a healthy dashboard shell instead of treating the dashboard URL alone as success, after the docs/search reruns showed API-authenticated tests could still land on a `500` dashboard during bootstrap.
   ...
   - shared `waitForDashboardReady()` in `wait-helpers.ts` no longer gates on raw `document.readyState`; it now polls one explicit authenticated-shell state (`main visible`, `search visible + enabled`, `no loading spinner`) before reasserting the shell, so auth/bootstrap/dashboard callers wait on the app surface they actually need instead of a generic document-complete signal; the focused `auth` + `dashboard` + `rbac` + `signout` rerun stayed green (`34 passed`, `3.8m`).
   - `SettingsPage.switchToTab()` no longer waits on raw `document.readyState`; it now uses a direct `goto(tab URL)` -> `toHaveURL(tab URL)` -> `selected tab visible` contract, and the settings save helper now lets the first-save attempt fail cleanly when a remount drops the dirty state so `toggleTimeApproval()` can use its explicit re-stage + second-save path; the focused `workspaces-org` rerun stayed green (`5 passed`, `49.5s`).
   - `BasePage.waitForLoad()` no longer relies on the raw React-fiber `waitForFunction()` probe; it now uses the real `body.app-hydrated` marker plus a visible-surface contract (`main` visible or at least one visible interactive control), so shared page objects wait for actual usable UI instead of generic internals; the focused `calendar` + `dashboard` + `documents` + `issues` rerun stayed green (`25 passed`, `2.8m`) after one unrelated onboarding-bootstrap retry.
   - shared `waitForConvexConnectionReady()` in `wait-helpers.ts` no longer uses `page.waitForFunction`; it now polls a named connection state derived from `getConvexConnectionInfo()` and honors the same optional hydration requirement, so auth/bootstrap, onboarding, and RBAC flows wait on one explicit Convex readiness contract instead of a raw browser function loop; the focused `auth` + `onboarding` + `rbac` + `signout` rerun stayed green (`30 passed`, `3.2m`).
   - dashboard modal dismissal now goes through one shared helper in `DashboardPage`: `closeGlobalSearchIfOpen()`, `closeAdvancedSearchIfOpen()`, `closeShortcutsHelpIfOpen()`, and `closeTimeEntryModalIfOpen()` now all use the same `Escape` -> bounded hidden wait -> optional fallback-dismiss contract instead of hand-rolling the same close logic four times; the focused `dashboard` + `search` + `billing` + `time-tracking` rerun stayed green (`22 passed`, `2.5m`).
   - `DashboardPage.openNotifications()` / `closeNotifications()` now use the same bounded overlay contract as the other dashboard surfaces: open waits for the notification panel to actually appear with one explicit second click, and close goes through the shared dismiss helper instead of a bare `Escape`; the focused `dashboard` + `search` + `billing` + `time-tracking` rerun stayed green (`22 passed`, `2.4m`).
   - shared workspace-dialog reset now lives in `dismissWorkspaceDialogIfOpen()` in `wait-helpers.ts`, and both `WorkspacesPage.createWorkspace()` and `ProjectsPage.createWorkspace()` use that same bounded Escape contract; after one failed rerun exposed that the workspaces flow still depends on the old blanket Escape reset, restoring that behavior inside the shared helper kept the duplication out and the focused `issues` + `teams` + `workspaces-org` rerun stayed green (`14 passed`, `2.5m`).
   - stale generic `document.readyState` waits are now removed from the shared E2E surface: unused `BasePage.waitForNavigation()`, `waitForReactHydration()`, `waitForNavigation()` in `wait-helpers.ts`, and `waitForNetworkIdle()` in `test-helpers.ts` are gone instead of lingering as copyable weak contracts; the mixed `calendar` + `dashboard` + `documents` + `issues` rerun stayed green (`25 passed`, `3.8m`).
   - `completeEmailVerification()` in `auth-helpers.ts` now uses the same bounded verification-submit contract as the auth page object: it waits for a real submit-start state, retries once only if submission never begins, and then waits for a named post-submit outcome (`redirect` / `success` / `error`) instead of a raw redirect wait; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.8m`).
   - post-auth destination handling in `auth-helpers.ts` now goes through one named destination-state contract (`dashboard` / `onboarding` / `pending`) instead of starting from raw `waitForURL(urlPatterns.dashboardOrOnboarding)` plus separate URL/body checks; `handleOnboardingOrDashboard()`, app-gateway recovery, and sign-up/bootstrap entry now share that state, and after one failed rerun showed the first version was too strict for API-login redirects, relaxing dashboard detection back to route-or-surface kept setup deterministic and the focused `auth` + `onboarding` + `signout` rerun stayed green (`27 passed`, `3.2m`).
   - `ensureSignInPage()` in `auth-helpers.ts` no longer hides its public-shell recovery behind `Promise.allSettled`; it now uses one explicit sign-in-link click plus a bounded route wait before falling back to direct `/signin` navigation, so the sign-in-route recovery path is readable and state-driven instead of “click and hope one branch settles”; the focused `auth` + `onboarding` + `signout` rerun stayed green again (`27 passed`, `3.4m`).
   - onboarding skip handling in `auth-helpers.ts` now goes through a named skip-control state (`button` / `link` / `text` / `pending`) instead of a swallowed boolean poll, and the second skip attempt now reacquires a visible control through the same helper instead of clicking the stale first locator with `.catch(() => {})`; the focused `auth` + `onboarding` + `signout` rerun stayed green (`27 passed`, `2.9m`).
   - `AuthPage.getAuthFormState()` now treats auth-form hydration as part of the state machine instead of swallowing `waitForAuthFormHydrated()` failures; when `data-hydrated` is not yet `true`, the form state stays `pending`, which keeps sign-in/sign-up/password-reset readiness polls honest without hiding hydration lag; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.1m`).
   - `AuthPage.gotoForgotPassword()` no longer goes through the generic `waitForHydration()` fallback; it now uses a route-specific forgot-password entry contract (`toHaveURL(/forgot-password/)` plus `getPasswordResetEntryState() === "forgot"` and a visible email input), so direct forgot-password navigation waits for the actual reset-entry surface instead of broad auth-page rendering heuristics; the focused `auth` + `auth-comprehensive` rerun stayed green again (`28 passed`, `3.1m`).
   - `AuthPage.ensureForgotPasswordEntry()` now returns the settled reset-entry state (`forgot` / `check-email` / `code`) instead of forcing callers to poll the same state again after it already finished waiting, so `requestPasswordReset()` and `submitPasswordResetRequest()` reuse one explicit route/state contract instead of duplicating the “persisted step or entry form?” branch; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `2.9m`).
   - `AuthPage.requestPasswordReset()` now routes its bounded second attempt through a named `retryPasswordResetRequest()` helper instead of open-coding `goto -> ensure entry -> resubmit` inline, so the password-reset request flow keeps one explicit recovery contract when the first request does not advance to the code step; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.1m`).
   - repeated auth submit-start waits in `AuthPage` now go through one shared `waitForSubmitStateStart()` helper instead of four separate try/catch poll wrappers for sign-up, sign-in, verification, and password reset, so those flows still have distinct state getters but no longer duplicate the same submit-start control flow; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.0m`).
   - forgot-password, verification, code-step, and submit-start waits in `AuthPage` now share one generic `waitForState()` / `waitForStateReady()` contract instead of scattering duplicate `expect.poll(...).not.toBe("pending")` wrappers, and the password-reset code-step helper now waits for the actual `code` state rather than treating intermediate reset surfaces as good enough before separately checking the input; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.1m`).
   - auth-form landing and expanded-state waits in `AuthPage` now also go through the same shared `waitForState()` / `waitForStateReady()` contract instead of keeping separate `expect.poll(() => getAuthFormState())` wrappers for `signin/signup` landing and expanded modes, so the auth shell transition helpers now use one consistent named-state polling path; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.0m`).
   - sign-up credential fill stability in `AuthPage` now goes through a named `getSignUpCredentialsFillState()` contract instead of a dedicated try/catch wrapper around `toHaveValue()` assertions, so the two-attempt sign-up fill path reuses the same state-driven readiness pattern as the rest of the auth page; the focused `auth` + `auth-comprehensive` rerun stayed green (`28 passed`, `3.0m`).
...
- `pnpm exec playwright test e2e/auth.spec.ts e2e/dashboard.spec.ts e2e/rbac.spec.ts e2e/signout.spec.ts --reporter=line --workers=1`
  - `34 passed (3.8m)` after replacing the shared dashboard-ready helper’s raw document-readiness gate with an explicit authenticated-shell readiness contract
- `pnpm exec playwright test e2e/workspaces-org.spec.ts --reporter=line --workers=1`
  - `5 passed (49.5s)` after replacing the settings tab-switch raw document-readiness wait with a direct URL/selected-tab contract and letting the first-save path fall back cleanly into the existing re-stage + second-save flow
- `pnpm exec playwright test e2e/dashboard.spec.ts e2e/documents.spec.ts e2e/calendar.spec.ts e2e/issues.spec.ts --reporter=line --workers=1`
  - `25 passed (2.8m)` after replacing the shared base-page load helper’s React-fiber probe with `body.app-hydrated` plus a visible-surface readiness contract; one earlier rerun died during onboarding auth bootstrap before the specs started, and the immediate retry completed cleanly
- `pnpm exec playwright test e2e/auth.spec.ts e2e/onboarding.spec.ts e2e/signout.spec.ts e2e/rbac.spec.ts --reporter=line --workers=1`
  - `30 passed (3.2m)` after replacing the shared Convex connection `waitForFunction` loop with a polled connection-state contract based on `getConvexConnectionInfo()`
- `pnpm exec playwright test e2e/dashboard.spec.ts e2e/search.spec.ts e2e/settings/billing.spec.ts e2e/time-tracking.spec.ts --reporter=line --workers=1`
  - `22 passed (2.5m)` after collapsing dashboard modal dismissals onto one shared `Escape`/fallback contract for global search, advanced search, shortcuts help, and time entry
- `pnpm exec playwright test e2e/dashboard.spec.ts e2e/search.spec.ts e2e/settings/billing.spec.ts e2e/time-tracking.spec.ts --reporter=line --workers=1`
  - `22 passed (2.4m)` after moving notifications onto the same bounded open/close overlay contract as the other dashboard surfaces
