...
function CalendarContextProbe() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => context.onEventMove(context.events[0], new Date("2026-03-14T00:00:00.000Z"))}
      >
        move event
      </button>
...
describe("CalendarProvider", () => {
  ...
  function renderProvider(overrides?: { calendarIconIsToday?: boolean; mode?: Mode }) {
    ...
    return { setMode, setDate, onAddEvent, onEventMove, onEventClick };
  }

  it("provides calendar context values and callbacks to children", async () => {
    const user = (await import("@testing-library/user-event")).default.setup();
    const { setMode, setDate, onAddEvent, onEventMove, onEventClick } = renderProvider({
      mode: "day",
    });
    ...
    expect(onEventMove).toHaveBeenCalledWith(event, new Date("2026-03-14T00:00:00.000Z"));
